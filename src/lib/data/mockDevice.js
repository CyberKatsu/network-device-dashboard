/**
 * mockDevice.js — Static seed data for ONU population
 *
 * Represents what would be returned by GET /api/ports/{id}/onus
 * on a real OLT via RESTCONF or NETCONF.
 *
 * ONU serial numbers follow ITU-T G.984 format: 4-char vendor ID + 8 hex digits
 */

function onu(id, vendor, status, rxPower, distance, firmware, profile) {
  const vendorCodes = { 'COMS': 'CommScope', 'ADTN': 'ADTRAN', 'HWTC': 'Huawei', 'ZTED': 'ZTE', 'BRCM': 'Broadcom OEM' }
  return {
    id,
    serial:    `${vendor}${id.toString(16).padStart(8, '0').toUpperCase()}`,
    vendor:    vendorCodes[vendor] ?? vendor,
    status,
    rxPower,   // dBm — received at OLT
    txPower:   2.0,  // dBm — ONU transmit (typical)
    distance:  distance,  // metres (calculated from ranging)
    firmware,
    profile,
    registeredAt: new Date(Date.now() - Math.random() * 86400000 * 30).toISOString(),
  }
}
...
export const ACTIVE_ONUS = PORTS.reduce(
  (s, p) => s + p.onus.filter(o => o.status === 'up').length, 0
)
